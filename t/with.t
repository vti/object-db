...
subtest 'convert with to joins with custom columns right order' => sub {
    for(1 .. 100){
	my $with = ObjectDB::With->new(meta => Book->meta, with => [ 
									
									{ name => 'parent_author', columns => [qw/name/]},
									{ name => 'description.parent_book', columns => [qw/id title/] },
									'parent_author.books',
